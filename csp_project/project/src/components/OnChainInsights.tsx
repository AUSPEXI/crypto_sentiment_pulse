...
const OnChainInsights: React.FC<OnChainInsightsProps> = ({ selectedCoins }) => {
  ...
  }, [selectedCoins.join(',')]); // Use a stable dependency to prevent unnecessary re-fetches
  ...
  const growthChart = {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: activeWalletsData.map(item => item.growth >= 0 ? '#22C55E' : '#EF4444'), // Green for positive, red for negative
