...
interface OnChainInsightsProps {// src/components/OnChainInsights.tsx
import React, { useState, useEffect } from 'react';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, ReferenceLine } from 'recharts';
import { fetchOnChainData } from '../utils/api';
import { OnChainData } from '../types';
...
const OnChainInsights: React.FC<OnChainInsightsProps> = ({ selectedCoins }) => {
  ...
  const GrowthBar = (props: any) => {
    ...
    const xPos = centerX; // Start all bars at 0% mark
    const adjustedWidth = value >= 0 ? barWidth : -barWidth; // Extend right for positive, left for negative

    return <rect x={xPos} y={y} width={Math.max(1, Math.abs(adjustedWidth))} height={height} fill={color} />; // Ensure minimum width of 1px
  };
  ...
  const getGrowthAxisRange = (data: typeof activeWalletsData) => {
    ...
    // Ensure a minimum range of ±20%, but expand if the data exceeds this
    const range = Math.max(20, maxAbsValue);
